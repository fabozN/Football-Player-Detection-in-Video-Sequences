...
def evaluate_method_yolo(pred_dir, gt_dir, img_size=(1920,1080)):
    ...
    plt.title("Precision-Recall Curve (YOLO roi)")
    plt.grid(True)
    plt.savefig("results/yolo_roi/sequence/PR_curve.png", dpi=200)
    plt.close()
    print("Grafico PR salvato come pr_curve_yolo.png")

    with open("results/yolo_roi/sequence/sequence.csv", "w", newline="") as f:
        writer = csv.writer(f)
...
if __name__ == "__main__":
    ...
    precision, recall, ap = evaluate_method_yolo(
        pred_dir="data/detections/yolo_finetuned/test/sequence",
