...
CONFIG = {
    "fixed": {
        ...
        "morph": {
            "kernel_open_field": (3,3),
            "kernel_close_field": (5,5),
            "kernel_expand": (9,9),
            "kernel_open_motion": (3,3),
            "kernel_close_motion": (3,3),
...
def detect_players(frame_bgr, cfg, debug_dir, frame_name=None, save_debug=False):
    ...
    mask_field = cv2.inRange(hsv_frame, cfg["lower_green"], cfg["upper_green"])

    kernel_open_field = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, morph["kernel_open_field"])
    kernel_close_field = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, morph["kernel_close_field"])

    if morph["apply_open_field"]:
        mask_field = cv2.morphologyEx(mask_field, cv2.MORPH_OPEN, kernel_open_field)
    ...
    field_mask_expanded = cv2.morphologyEx(field_mask_clean, cv2.MORPH_CLOSE, kernel_expand)

    mask_motion = fgbg.apply(frame_bgr)
    kernel_open_motion = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, morph["kernel_open_motion"])
    kernel_close_motion = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, morph["kernel_close_motion"])
    if morph["apply_open_motion"]:
        mask_motion = cv2.morphologyEx(mask_motion, cv2.MORPH_OPEN, kernel_open_motion)
    if morph["apply_close_motion"]:
        mask_motion = cv2.morphologyEx(mask_motion, cv2.MORPH_CLOSE, kernel_close_motion)

    mask_players = cv2.bitwise_and(mask_motion, field_mask_expanded)
...
def step2_player_detection(sequence_name, save_debug=True, show_live=False):
    ...
    DETECTIONS_DIR = f"data/detections/mog2_solo_mask/{sequence_name}"
    DEBUG_DIR = f"debugs/debug_step2/mog2_solo_mask/{sequence_name}"
